    ...
    <div className="h-screen bg-[#0A0A0F] flex overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        {isPlaying && isTTRE && (() => {
          ...
          const handleKeepTickets = (keepTicketIds: string[]) => {
            hostController.handleLocalAction({ action: 'keep_tickets', keepTicketIds });
          };
          ...
          return (
            <div className="flex flex-col flex-1 overflow-hidden">
              ...
              {/* ── Bottom HUD: Reusable Overhauled Player Dashboard ── */}
              {hostPlayer && (
                <PlayerHUD
                  player={hostPlayer}
                  gameState={ttreData}
              ...
              {/* ── Destination Ticket Selection Modal Overlay ── */}
              {isHostTurn && hostPlayer && hostPlayer.pendingTickets && hostPlayer.pendingTickets.length > 0 && (
                <TicketModal
                  pendingTickets={hostPlayer.pendingTickets}
                  onConfirm={handleKeepTickets}
                />
              )}
            </div>
          );
